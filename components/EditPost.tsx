...
function EditPost({ id, post }: { id: string; post: Post }) {
  ...
  const isEditPage = pathname === `/painel/p/${id}/edit`;
  ...
  <AspectRatio ratio={1 / 1} className="relative h-full">
    {fileUrls.length > 0 ? (
      <Image
        src={fileUrls[0]} // Exibe apenas a primeira imagem do array
        alt="Post preview"
        fill
        className="rounded-md object-cover"
      />
    ) : (
      <p>Sem imagens disponíveis</p> // Mensagem caso não haja imagens
    )}
  </AspectRatio>
</div>
