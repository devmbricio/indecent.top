...
export default function PowerballMundi({ isInlineAd = false }: PowerballMundiProps) {
    ...
    useEffect(() => {
        const fetchPastResults = async () => {
            ...
            console.log("Iniciando busca por resultados anteriores...");
            try {
                const res = await fetch("/api/powerball/scrape-past-results?limit=5"); // Buscar os últimos 5 sorteios raspados
                console.log("Resposta da busca por resultados anteriores:", res);
                if (!res.ok) {
                    const data = await res.json();
                    console.error("Erro na resposta da busca por resultados anteriores:", data);
                    throw new Error(data?.error || "Erro ao buscar resultados anteriores");
                }
                const data: PowerballResult[] = await res.json();
                console.log("Dados dos resultados anteriores recebidos:", data);
                setPastResults(data);
            } catch (err: any) {
                setError("Erro ao buscar resultados anteriores");
                console.error("Erro ao buscar resultados anteriores (catch):", err);
                setPastResults([]);
            } finally {
                setFetchingPastResults(false);
                console.log("Finalizando busca por resultados anteriores.");
            }
        };
    ...
    useEffect(() => {
        const fetchCredits = async () => {
            console.log("Iniciando busca por créditos...");
            try {
                const res = await fetch("/api/user/credits");
                console.log("Resposta da busca por créditos:", res);
                if (!res.ok) {
                    const data = await res.json();
                    console.error("Erro na resposta da busca por créditos:", data);
                ...
                console.error("Erro ao buscar créditos (catch):", err);
    ...
    useEffect(() => {
        const fetchLatestResults = async () => {
            ...
            console.log("Iniciando busca pelos últimos resultados...");
            try {
                const res = await fetch("/api/powerball/latest-results");
                console.log("Resposta da busca pelos últimos resultados:", res);
                if (!res.ok) {
                    const data = await res.json();
                    console.error("Erro na resposta da busca pelos últimos resultados:", data);
                    throw new Error(data?.error || "Erro ao buscar resultados");
                }
                const data: PowerballResult = await res.json();
                console.log("Últimos resultados recebidos:", data);
                setLatestResult(data);
            } catch (err: any) {
                setError("Erro ao buscar últimos resultados");
                console.error("Erro ao buscar últimos resultados (catch):", err);
                setLatestResult(null);
            } finally {
                setFetchingResults(false);
                console.log("Finalizando busca pelos últimos resultados.");
            }
        };
    ...
    useEffect(() => {
        const fetchJackpotInfo = async () => {
            ...
            console.log("Iniciando busca por informações do jackpot...");
            try {
                const res = await fetch("/api/powerball/jackpot-info");
                console.log("Resposta da busca por informações do jackpot:", res);
                if (!res.ok) {
                    const data = await res.json();
                    console.error("Erro na resposta da busca por informações do jackpot:", data);
                    throw new Error(data?.error || "Erro ao buscar informações do jackpot");
                }
                const data: JackpotInfo = await res.json();
                console.log("Informações do jackpot recebidas:", data);
                setJackpotInfo(data);
            } catch (err: any) {
                setError("Erro ao buscar informações do jackpot");
                console.error("Erro ao buscar informações do jackpot (catch):", err);
                setJackpotInfo(null);
            } finally {
                setFetchingJackpot(false);
                console.log("Finalizando busca por informações do jackpot.");
            }
        };
    ...
    useEffect(() => {
        const fetchPastResults = async () => {
            setFetchingPastResults(true);
            console.log("Iniciando (segunda) busca por resultados anteriores...");
            try {
                const res = await fetch(`/api/powerball/past-results?limit=5`); // Buscar os últimos 5 sorteios salvos
                console.log("Resposta da (segunda) busca por resultados anteriores:", res);
                if (!res.ok) {
                    const data = await res.json();
                    console.error("Erro na resposta da (segunda) busca por resultados anteriores:", data);
                    throw new Error(data?.error || "Erro ao buscar resultados anteriores");
                }
                const data: PowerballResult[] = await res.json();
                console.log("(Segundos) Dados dos resultados anteriores recebidos:", data);
                setPastResults(data);
            } catch (err: any) {
                setError("Erro ao buscar resultados anteriores");
                console.error("Erro ao buscar resultados anteriores (segundo catch):", err);
                setPastResults([]);
            } finally {
                setFetchingPastResults(false);
                console.log("Finalizando (segunda) busca por resultados anteriores.");
            }
        };
    ...
    const handlePlaceBet = async () => {
        ...
        setBetting(true);
        setError("");
        console.log("Iniciando aposta...", { mainNumbers, powerballNumber, quantity });
        ...
        try {
            ...
            console.log("Resposta da aposta:", res);
            const data: BetResponse = await res.json();
            console.log("Dados da resposta da aposta:", data);
            ...
            console.error("Erro ao apostar (catch):", err);
        } finally {
            setBetting(false);
