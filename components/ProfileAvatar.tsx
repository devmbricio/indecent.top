...
function ProfileAvatar({
  ...
  render={({ field }) => (
    <FormItem>
      <FormControl>
        <UploadAvatarButton
          className="text-sm h-11 bg-transparent border-y border-zinc-300 dark:border-neutral-700 text-blue-500 font-bold ring-0 focus-visible:ring-0 ring-offset-0 w-full"
          endpoint="imageUploader"  // Endpoint específico para o upload de imagem de perfil
          onUploadComplete={(res) => {
            form.setValue("image", res[0]);  // Atualiza o campo 'image' do formulário
            if (inputRef.current) {
