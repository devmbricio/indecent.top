  ...
  return (
    <div className="space-y-0.5 px-3 sm:px-0">
      {commentsCount > 1 && (
        <Link
          scroll={false}
          href={`/painel/p/${postId}`}
          className="text-sm font-medium text-neutral-500"
        >
          Ver todos {commentsCount} comentários
        </Link>
      )}
      ...
      {optimisticComments.slice(0, 3).map((comment, i) => {
        ...
        return (
          ...
          >
            <Link href={`/painel/${username}`} className="font-semibold">
              {username}
            </Link>
            <p>{comment.body}</p>
          </div>
      ...
      <Form {...form}>
        ...
        >
              ...
              <FormItem className="w-full flex">
                <FormControl>
                  <input
                    ...
                    className="bg-transparent text-sm border-none focus:outline-none flex-1 placeholder-neutral-500 dark:text-[#ddc897] dark:placeholder-neutral-400 font-medium"
                    {...field}
                  />
                </FormControl>
                <FormMessage />
          ...
          {body.trim().length > 0 && (
            <button
              type="submit"
              className="text-sky-500 text-sm font-semibold hover:text-[#ddc897] disabled:hover:text-sky-500 disabled:cursor-not-allowed"
            >
              Post
            </button>
          )}
