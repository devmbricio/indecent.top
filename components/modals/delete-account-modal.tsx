  ...
  return (
    ...
    >
      <div className="flex flex-col items-center justify-center space-y-3 border-b p-4 pt-8 sm:px-16">
        <UserAvatar
          user={{
            name: session?.user?.name ?? "", // Garante que seja uma string
            image: session?.user?.image ?? "", // Garante que seja uma string
          }}
        />
      ...
      >
        <div>
          <label htmlFor="verification" className="block text-sm">
            To verify, type{" "}
            <span className="font-semibold text-black dark:text-[#ddc897]">
