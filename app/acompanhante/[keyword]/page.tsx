...
export async function generateMetadata({ params }: { params: { country: string; city: string } }) {
  ...
  const title = `Encontre acompanhante em ${formattedCity}, ${formattedCountry} | Indecent Top`;
  const description = `Encontre acompanhante em ${formattedCity}, ${formattedCountry}. Mulheres, homens e trans de luxo. Perfis com fotos, descrição, valor, idade e WhatsApp.`;

  return {
    title,
    description,
    keywords: [
      `acompanhante ${formattedCity}`,
      `acompanhante ${formattedCountry}`,
      "acompanhante de luxo",
      "acompanhante independentes",
      "acompanhante trans",
      "acompanhante verificados",
    ],
    alternates: {
    ...
    />
        ...
        <div className="text-center mt-10 space-y-2">
          ...
          {cidadesSugestao.map((cidade) => (
            <div key={cidade.slug}>
              <Link
                href={`/acompanhante/${cidade.slug}/brasil`}
