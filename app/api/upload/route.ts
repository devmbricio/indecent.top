...
export async function POST(request: Request) {
  try {
    ...
    // Nome do bucket
    const bucketName = process.env.AWS_AWS_BUCKET_NAME;
    const AWS_REGION = process.env.AWS_AWS_REGION;

    if (!bucketName || !AWS_REGION) {
      return NextResponse.json(
        { error: "As variáveis de ambiente AWS_AWS_BUCKET_NAME e AWS_AWS_REGION são obrigatórias." },
        { status: 500 }
      );
    }
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      uploadUrl,
      fileUrl: `https://${process.env.AWS_AWS_BUCKET_NAME}.s3.${process.env.AWS_AWS_REGION}.amazonaws.com/${params.Key}`,
    });
  } catch (error) {
    console.error("Erro ao gerar URL assinada:", error);
    return NextResponse.json(
